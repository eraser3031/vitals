    ...
    <ul className="list-none m-0 p-2 overflow-y-auto flex-1">
      {reports.map(report => {
        ...
        return (
          <li
            key={report.filename}
            className={`px-3 py-2.5 rounded-lg cursor-pointer mb-0.5 transition-colors ${isSelected ? 'bg-neutral-100' : 'hover:bg-hover-bg'}`}
          ...
          >
            ...
            <div className="flex gap-2 mt-1 pl-6">
              ...
              {report.meta.date && (
                <span className="text-[11px] text-muted">{report.meta.date}</span>
              )}
            </div>
          </li>
        )
